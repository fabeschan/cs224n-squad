...
class QASystem(object):
    ...
    def setup_system2(self):
        ...
        # Add embeddings for question and paragraph
        self.embedding_mat = tf.Variable(self.pretrained_embeddings, name = "pre", dtype=tf.float32, trainable=False)
        ...
        with tf.variable_scope("encode_q"):
            (output_q_fw, output_q_bw), _  = tf.nn.bidirectional_dynamic_rnn(
                cell_fw=cell,
                cell_bw=cell,
                inputs=self.q_emb,
        ...
        W_start = tf.get_variable("W_start", shape = [dim_final_layer, 1], initializer = tf.contrib.layers.xavier_initializer())
        ...
        W_end = tf.get_variable("W_end", shape = [dim_final_layer, 1], initializer = tf.contrib.layers.xavier_initializer())
    ...
    def setup_loss_enriched_ensemble(self):
        ...
        # Someone replaced yp_start with logits_start in loss. Don't really follow the change. Setting it back to original.
        with vs.variable_scope("loss"):
            self.loss_start_1 = (tf.reduce_mean(
                tf.nn.softmax_cross_entropy_with_logits(logits = self.logits_start_1, labels = tf.cast(self.a_start, tf.float32))))
            self.loss_end_1 = (tf.reduce_mean(
                tf.nn.softmax_cross_entropy_with_logits(logits = self.logits_end_1, labels = tf.cast(self.a_end, tf.float32))))
            self.loss_start_2 = (tf.reduce_mean(
                tf.nn.softmax_cross_entropy_with_logits(logits = self.logits_start_2, labels = tf.cast(self.a_start, tf.float32))))
            self.loss_end_2 = (tf.reduce_mean(
                tf.nn.softmax_cross_entropy_with_logits(logits = self.logits_end_2, labels = tf.cast(self.a_end, tf.float32))))
